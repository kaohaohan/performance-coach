...
- Data flow:
  ...
  6. A separate `/bootstrap` Job may run only after a reviewed manifest is supplied; it uses the same migration identity and direct endpoint, but receives `BOOTSTRAP_MANIFEST_PATH` pointing to a reviewed non-secret manifest file delivered into the container by a separately approved mechanism.
- Job configuration:
...
- Bootstrap manifest contract:
  - The file is one JSON object with exactly the top-level fields `users` and `relationships`; unknown fields are rejected by `DisallowUnknownFields`.
  - Each `users` element requires `firebaseUid`, `name`, and `role`, where `role` is exactly `COACH` or `ATHLETE`. `firebaseUid` values must be unique.
  - Each `relationships` element requires `coachFirebaseUid` and `athleteFirebaseUid`. Both UIDs must be declared in `users`, and their declared roles must be `COACH` and `ATHLETE` respectively.
  - The command requires at least one user. An empty relationship list is structurally allowed; no relationship may reference an undeclared or incorrectly typed user.
  - IDs and timestamps are generated by the command: each inserted `users.id` is a new UUID and `created_at` is the current UTC time. Relationship IDs are not supplied; `coach_athletes` uses the two generated user IDs and its composite primary key.
  - The reviewed placeholder template is:
    ```json
    {
      "users": [
        {
          "firebaseUid": "<APPROVED_COACH_FIREBASE_UID>",
          "name": "<APPROVED_COACH_DISPLAY_NAME>",
          "role": "COACH"
        },
        {
          "firebaseUid": "<APPROVED_ATHLETE_FIREBASE_UID>",
          "name": "<APPROVED_ATHLETE_DISPLAY_NAME>",
          "role": "ATHLETE"
        }
      ],
      "relationships": [
        {
          "coachFirebaseUid": "<APPROVED_COACH_FIREBASE_UID>",
          "athleteFirebaseUid": "<APPROVED_ATHLETE_FIREBASE_UID>"
        }
      ]
    }
    ```
- Bootstrap behavior:
  - `bootstrap.Apply` opens one transaction, performs all user upserts and relationship inserts, and commits once. Errors before commit roll back all writes; it does not intentionally leave partial writes. A commit can succeed even if a caller loses the response, so a later rerun is the recovery path.
  - Re-running the same manifest is idempotent: users upsert by unique `firebase_uid` and re-affirm `name`/`role`; relationships use `ON CONFLICT DO NOTHING`. It is therefore safe to run once and safe to repeat after an ambiguous job result, but it is not a no-op if an approved manifest intentionally changes a user's name or role.
  - `uuid.NewString()` means a conflicting user upsert still generates an unused UUID; it does not replace the existing primary key.
- Schema changes:
...
| Gate 4 — execute and verify migration | Done | `performance-coach-migrate-kftdc` completed successfully once; schema, ledger rows, and exact checksums verified without reading secrets. |
| Gate 5 — bootstrap manifest review | Done | `/bootstrap` input contract and placeholder-only template reviewed. Execution remains blocked until production values and a file-delivery mechanism are approved. |
| Gate 6 — bootstrap Job and verification | Blocked | Requires approved Firebase UIDs, names, roles, relationships, and a reviewed way to provide the non-secret manifest at `BOOTSTRAP_MANIFEST_PATH`. |
...
- Final status: D3c migration is complete and bootstrap preparation is documented; bootstrap execution is blocked pending approved production values and manifest delivery.
- Deviations from plan: None.
- Follow-ups:
  - Obtain explicit approval for every production Firebase UID, display name, role, and coach-athlete relationship in the placeholder template.
  - Select and provision a least-privilege non-secret manifest file-delivery mechanism that is compatible with `BOOTSTRAP_MANIFEST_PATH`; do not place manifest content in an environment variable or image layer.
