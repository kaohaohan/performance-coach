...
- **Prescription scope** — approved V0.1 planned-set behavior is limited to ordered positions, uniform defaults, individual overrides, reps or an existing text prescription, load with unit where present, and RPE. Percentages, velocity, tempo, rest, supersets, circuits, arbitrary custom properties, and TeamBuildr-style property systems remain out of scope. Storage and API representation remain pending architecture design.
