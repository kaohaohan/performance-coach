...
| Deployment + physical-device verification | In Progress | Local verification completed: focused/full web tests, webpack production build, production Capacitor sync/plugin registration, plist/AASA checks, and signed generic iOS Release build passed. Signed app entitlements include `applinks:dontworkout.vercel.app` and Sign in with Apple; production AASA deployment and Safari/Messages/Mail/LINE physical-device checks remain external. |
...
- Follow-ups: Verify production AASA deployment and Safari/Messages/Mail/LINE physical-device behavior; the local signed Release used the existing Apple Development profile and contained the required Associated Domains and Sign in with Apple entitlements. Android App Links remain deferred until an Android target exists.
