...
- Existing Workout path: choose a saved Workout and assign it to the selected Athlete(s). When the editor is opened from an Athlete's Calendar, that Athlete is the mandatory assignment source (checkbox selected and not clearable). Additional Athletes may be checked explicitly. Assign submits the same Athlete IDs the Assign-to checkboxes show.
- Inline Build path: build one ordered Workout draft from existing Exercises, validate it, `POST /api/v1/workouts` once, retain the returned `workout.id`, then `POST /api/v1/scheduled-workouts` once with all selected Athlete IDs and the selected date. Refresh Calendar assignment state and Workout choices after success. A builder opened from Athlete X's Calendar always includes X as the assignment source; extra Athletes are optional. A restored Build draft keeps that source identity and must not silently assign to a different Athlete than the builder UI names. Switching Calendar context while a live draft exists does not retarget that draft. Build & Assign submits the same Athlete IDs the Assign-to checkboxes show.
...
- Discard Draft clears the browser-local Build draft only. It issues no request and never removes a scheduled workout; Remove is the undo for an assignment.
