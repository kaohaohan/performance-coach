...
- Affected files/components:
  - `apps/web/capacitor.config.ts` — `appId` (`com.performancecoach.app` → `com.pumpslate.app`) and `appName` (`Performance Coach` → `PumpSlate`). This is Capacitor's source of truth: `npx cap sync ios` propagates `appId` into the native project, so changing `project.pbxproj` without changing this would be undone on the next sync. Change this **first**, then sync, then verify the native values.
  - `apps/web/ios/App/App.xcodeproj/project.pbxproj` — `PRODUCT_BUNDLE_IDENTIFIER` from `com.performancecoach.app` to `com.pumpslate.app` (Debug + Release target configs; expect `cap sync` to handle this, verify rather than hand-edit); once Apple Developer access exists, set `DEVELOPMENT_TEAM`; **attach a Release xcconfig to the two Release `XCBuildConfiguration` entries that currently have none** (see Risks above) — either extend `debug.xcconfig`'s coverage or add a parallel `release.xcconfig`, decision deferred to implementation since it doesn't change the design shape. `cap sync` will *not* do this part; it is a manual project edit.
  ...
  - **Deliberately NOT changed by this task — flagged for a separate decision:** ten web pages render a visible `Performance Coach` brand label (`apps/web/app/login/page.tsx:100`, `app/today/page.tsx:131`, `app/join/page.tsx:41`, `app/join/[code]/page.tsx:362`, `app/coach/signup/page.tsx:200`, and `app/coach/{calendar,clients,clients/[athleteId],exercises,workouts}/page.tsx`). Because the iOS shell loads the web app through `server.url`, **beta testers will see "Performance Coach" on every screen inside an app named PumpSlate.** That is a real brand inconsistency, but it is web-app copy, not authentication, and rebranding user-visible UI is a scope expansion this task has no mandate for (AGENTS.md §3). Raise it with the user as its own small task before the beta goes out; do not fold it in here.
  - `apps/web/capacitor-shell/index.html` also says `Performance Coach`, but it is the bundled placeholder that `server.url` overrides at runtime — cosmetic only, not user-visible in the current configuration.
...
- Sub-task breakdown:
  ...
  3. Update `capacitor.config.ts` (`appId`, `appName`), run `npx cap sync ios`, then verify `project.pbxproj` `PRODUCT_BUNDLE_IDENTIFIER` and set `Info.plist` `CFBundleDisplayName` → `PumpSlate`. Set `DEVELOPMENT_TEAM` once Apple access is available.
  4. Fix the Release `XCBuildConfiguration` xcconfig gap (manual project edit — `cap sync` will not do it); update `GOOGLE_REVERSED_CLIENT_ID` for the new Firebase app.
...
- Final status: Task Doc complete through Feasibility Analysis and Technical Design (§1–§3). **Phase 6a implementation done** (this session, remote/Linux):
  - `apps/web/capacitor.config.ts`: `appId` → `com.pumpslate.app`, `appName` → `PumpSlate`.
  - `npx cap sync ios` run: propagated `appId`/`appName` into the gitignored `ios/App/App/capacitor.config.json`, as expected. Confirmed by inspection that `cap sync` does **not** touch `PRODUCT_BUNDLE_IDENTIFIER` in `project.pbxproj` — that stayed `com.performancecoach.app` after sync and was hand-edited (both Debug and Release target-level `XCBuildConfiguration` entries) to `com.pumpslate.app`, matching §2's expectation that this be verified rather than assumed.
  - `apps/web/ios/App/App/Info.plist`: `CFBundleDisplayName` → `PumpSlate`.
  - Release xcconfig gap closed by **adding a new `apps/web/ios/release.xcconfig`** (chose the new-file option over extending `debug.xcconfig`: `CAPACITOR_DEBUG` must legitimately differ between Debug (`true`) and Release (`false`), so sharing one xcconfig file across both configurations would have forced them to the same value — a new file matching the existing `debug.xcconfig` pattern keeps the per-configuration semantics correct). Wired into `project.pbxproj` via a new `PBXFileReference` and `baseConfigurationReference` on both Release `XCBuildConfiguration` entries (`504EC3151FED79650016851F` project-level, `504EC3181FED79650016851F` target-level). `GOOGLE_REVERSED_CLIENT_ID` value deliberately left unchanged (still the `com.performancecoach.app` client ID) — updating it is sub-task 4's Firebase-dependent half, out of scope until the new Firebase iOS app is registered (see Dependencies #2 below); a `TODO` comment in the new file points back to this doc.
  - Not touched, per explicit scope: `DEVELOPMENT_TEAM` (needs Apple Developer access), `GOOGLE_REVERSED_CLIENT_ID` value, the ten web pages' "Performance Coach" brand strings, and no archive/upload/App Store Connect actions were taken.
- Verification performed in this environment (remote/Linux, no Xcode):
