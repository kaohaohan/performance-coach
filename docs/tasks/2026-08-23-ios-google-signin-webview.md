...
| Phase 6 — Ship both halves | Done | iOS app `com.performancecoach.app` registered in the `dontworkout` Firebase project; `NEXT_PUBLIC_GOOGLE_IOS_CLIENT_ID` set in Vercel (Preview+Production) and `GOOGLE_REVERSED_CLIENT_ID` set in `debug.xcconfig` (commit `97e49f2`); this branch merged into `staging` (merge commit `6669d82`, later carried forward through `cf34987`) and deployed — confirmed the staging alias serves the new build. Also required and fixed along the way: staging's Firebase project was missing the staging hostname in Authentication → Settings → Authorized domains (`auth/unauthorized-domain`), added by the user. |
| Phase 7 — On-device sign-in verification | Done | User confirmed successful Google sign-in **both** in the iOS Simulator (native `@capgo/capacitor-social-login` branch) and in a browser incognito window (web `signInWithPopup` branch, unaffected by this change) — reached the athlete "Today" screen in both. |

## 5. Outcome (filled at completion)

- Final status: **Complete and verified end-to-end.** Google sign-in works both natively in the iOS Simulator and on the web, on the deployed staging alias.
- Deviations from plan: None from the selected option (§1). Operationally, shipping required more than the code change: a stray Xcode/SPM cache corruption (mid-resolution `npm ci` had reset the plugin's local provider-trimming, then a partial GUI resolve poisoned the workspace `Package.resolved`) needed a full DerivedData + SwiftPM cache wipe to resolve, and the Firebase project was also missing the staging hostname in Authorized domains — neither was anticipated in the original design.
- Follow-ups: Full Firebase isolation (a dedicated non-shared Firebase project for staging) remains out of scope per `docs/tasks/2026-08-21-preview-backend-for-scheduled-workout-edit.md` — this task did not change that. Apple/Facebook/Twitter sign-in remain disabled (`providers` config) and out of scope.
