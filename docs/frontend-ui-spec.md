...
- A copied Workout uses these same uniform-first rules. The Coach may change one copied exercise's set count or default load/reps/RPE without retyping every set; positions with an explicit override continue to follow override semantics. Each exercise may also set a per-exercise weekly `loadIncrement` (`kg`: 0 / 2.5 / 5 / 10; `lb`: 0 / 5 / 10; defaults 2.5 kg / 5 lb) and optional weekly `setIncrement` (`0` or `1`, default `0`). When exactly one Athlete is selected in the Builder, copy/prefill may suggest bumped loads from the last COMPLETED SetLog for that exercise and unit, falling back to the template's planned load when no history exists, and bumped set counts from `setIncrement`. Multi-Athlete assign keeps template-written loads and set counts. **Week view — Repeat this week:** when exactly one Athlete is selected and the visible week has at least one assignment, Coach may repeat the week forward (+7 days). Preview lists each session (source date → target date, workout name, suggested load/set summary, conflict flag when the target date already has assignments). Confirm orchestrates existing `POST /api/v1/workouts` then `POST /api/v1/scheduled-workouts` per session sequentially; source templates and prior snapshots stay unchanged. Partial failure stops with a clear summary and reuses the created-but-unassigned retry pattern when scheduling fails after create. Workout-wide percentage progression and bulk cross-exercise set/load adjustments remain out of scope.
