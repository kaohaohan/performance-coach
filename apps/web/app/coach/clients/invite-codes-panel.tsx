...
export default function InviteCodesPanel({ idToken, reloadToken }: { idToken: string; reloadToken: number }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled && id === requestId.current) setLoadError(errorMessage(error));
      }
    })();
  ...
  return (
    <section>
      ...
      {codes === null ? (
        <section className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-950/5"><p className="text-sm font-medium text-slate-500">Loading invite codes…</p></section>
      ) : codes.length === 0 ? (
        <section className="rounded-3xl border border-dashed border-slate-200 bg-stone-50 px-5 py-6 text-center">
          <p className="font-semibold">No invite codes yet.</p>
          <p className="mt-1 text-sm text-slate-500">Use + Invite Athletes above to create one.</p>
        </section>
      ) : (
        <>
              ...
              <tbody className="divide-y divide-slate-100">
                {codes.map((code) => (
                  <tr key={code.id} className="align-top">
                    ...
                    <td className="px-5 py-3"><span className={`rounded-full px-2.5 py-1 text-[11px] font-bold tracking-wide ring-1 ${statusChipClass(code.status)}`}>{code.status}</span></td>
                    <td className="px-5 py-3">
                      <div className="flex flex-wrap justify-end gap-2">
                        <button type="button" onClick={() => handleCopy(`link-${code.id}`, inviteLink(code.code))} className="min-h-11 rounded-lg px-2.5 text-sm font-bold text-slate-700 transition hover:bg-stone-100">{copiedField === `link-${code.id}` ? "Copied ✓" : "Copy link"}</button>
                        <button type="button" onClick={() => handleCopy(`code-${code.id}`, formatCode(code.code))} className="min-h-11 rounded-lg px-2.5 text-sm font-bold text-slate-700 transition hover:bg-stone-100">{copiedField === `code-${code.id}` ? "Copied ✓" : "Copy code"}</button>
                        <button type="button" disabled={code.status !== "ACTIVE" || revokingId === code.id} onClick={() => setConfirmTarget(code)} className="min-h-11 rounded-lg px-2.5 text-sm font-bold text-red-700 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:text-slate-300 disabled:hover:bg-transparent">{revokingId === code.id ? "Revoking…" : "Revoke"}</button>
                      </div>
                    </td>
                  </tr>
                ))}
              ...
              <li key={code.id} className="rounded-3xl bg-white p-4 shadow-sm ring-1 ring-slate-950/5">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="font-semibold text-slate-900">{code.description ?? "Invite code"}</p>
                    <p className="mt-0.5 font-mono text-sm tracking-wide text-slate-600">{formatCode(code.code)}</p>
                  </div>
                  <span className={`shrink-0 rounded-full px-2.5 py-1 text-[11px] font-bold tracking-wide ring-1 ${statusChipClass(code.status)}`}>{code.status}</span>
                </div>
                <p className="mt-2 text-xs text-slate-500">Expires {formatDate(code.expiresAt)}</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  <button type="button" onClick={() => handleCopy(`link-${code.id}`, inviteLink(code.code))} className="min-h-11 flex-1 rounded-xl bg-stone-50 px-3 text-sm font-bold text-slate-700 transition hover:bg-stone-100">{copiedField === `link-${code.id}` ? "Copied ✓" : "Copy link"}</button>
                  <button type="button" onClick={() => handleCopy(`code-${code.id}`, formatCode(code.code))} className="min-h-11 flex-1 rounded-xl bg-stone-50 px-3 text-sm font-bold text-slate-700 transition hover:bg-stone-100">{copiedField === `code-${code.id}` ? "Copied ✓" : "Copy code"}</button>
                  <button type="button" disabled={code.status !== "ACTIVE" || revokingId === code.id} onClick={() => setConfirmTarget(code)} className="min-h-11 flex-1 rounded-xl bg-stone-50 px-3 text-sm font-bold text-red-700 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:text-slate-300 disabled:hover:bg-stone-50">{revokingId === code.id ? "Revoking…" : "Revoke"}</button>
                </div>
              </li>
            ))}
          </ul>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/40 p-4 sm:items-center" role="presentation" onClick={onCancel}>
      <div ref={containerRef} role="alertdialog" aria-modal="true" aria-labelledby="revoke-dialog-title" className="w-full max-w-sm rounded-3xl bg-white p-6 shadow-xl" onClick={(event) => event.stopPropagation()}>
        <h2 id="revoke-dialog-title" className="text-lg font-semibold tracking-tight">Revoke this invite code?</h2>
        <p className="mt-2 text-sm leading-6 text-slate-600">{code.description ? `“${code.description}” (${formatCode(code.code)})` : formatCode(code.code)} will stop working for anyone who hasn&apos;t already joined. Athletes already connected keep their spot.</p>
        <div className="mt-6 flex gap-3">
          ...
          <button type="button" onClick={onConfirm} className="min-h-14 flex-1 rounded-2xl bg-red-600 text-base font-bold text-white shadow-sm transition hover:bg-red-700">Revoke</button>
        </div>
      </div>
    </div>
  );
