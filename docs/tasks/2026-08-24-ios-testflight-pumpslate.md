...
| Beta authentication | Done | Firebase verification found no `com.pumpslate.app` registration, so created the new active PumpSlate iOS App in `dontworkout` while preserving the old `com.performancecoach.app` App. Updated `GOOGLE_REVERSED_CLIENT_ID`, added the matching public `NEXT_PUBLIC_GOOGLE_IOS_CLIENT_ID` only to Vercel Preview branch `staging`, and redeployed the staging alias successfully. Using the approved Personal Team Development profile, installed the signed `com.pumpslate.app` Debug build on the paired iPhone; Google login to staging completed successfully. |
