...
| Route | Role | Backs onto | Purpose |
|---|---|---|---|
| `/session/[id]` | Coach (connected) + Athlete (self) | `POST /scheduled-workouts/{id}/session`, `GET /sessions/{id}`, `POST/PATCH/DELETE /sessions/{id}/set-logs`, `POST /sessions/{id}/complete` | The Training Session UI. Both roles use the same screen/domain: start or resume a session, log/edit/delete SetLogs, view plan vs. actual, complete the session. Existing SetLogs remain editable in both ACTIVE and COMPLETED sessions by the Athlete or an active-relationship Coach; completed sessions hide/disable add and delete actions. PATCH preserves session status/`completedAt` and immutable SetLog associations. After an Athlete deletes their account, a Coach may still open historical sessions as read-only (`GET /sessions/{id}`); start/complete/set-log are refused. |
...
- The Session UI associates a normal actual SetLog with a specific frozen planned position while keeping server-assigned `setNumber` as actual chronology. Extra actual sets are allowed and displayed as EXTRA with no target. Planned positions without an actual log remain visibly incomplete; there is no persisted skipped-row UI in V0.1. Existing SetLogs are tappable for edit in ACTIVE or COMPLETED sessions when authorized; completed sessions prohibit adding or deleting logs.
...
- Coach and Athlete share the same Training Session UI/domain wherever the underlying authorization allows it (session start, set-log CRUD). In COMPLETED sessions, only PATCH of existing SetLogs is available; POST and DELETE remain unavailable.
- An ACTIVE ScheduledWorkout remains incrementally adjustable. Calendar Edit renders existing targets read-only and lets an active-relationship Coach add, soft-remove, or replace Exercises. The Athlete Session view lets the Athlete add a SYSTEM Exercise or one of the ScheduledWorkout Coach's private Exercises with a complete planned-set prescription, and remove/replace only their own additions. Athlete additions use an amber treatment plus an `Athlete added` text label; provenance is never communicated by color alone.
