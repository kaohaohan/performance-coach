...
- A copied Workout uses these same uniform-first rules. The Coach may change one copied exercise's set count or default load/reps/RPE without retyping every set; positions with an explicit override continue to follow override semantics. Workout-wide percentage progression, automatic load recommendations, and bulk cross-exercise set/load adjustments are not part of this slice.
