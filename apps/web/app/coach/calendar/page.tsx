...
type BuildFieldErrors = {
  ...
  name?: string;
  exercises?: string;
  items: Record<number, Partial<Record<"sets" | "reps" | "note" | "rpe", string>>>;
};
              ...
              <form onSubmit={handleBuildAndAssign} className="mt-4 grid gap-4">
                ...
                <div>
                  ...
                  <div className="mt-3 grid gap-4">
                    {draftExercises.map((item, index) => <DraftExerciseCard key={item.exercise.id} item={item} index={index} total={draftExercises.length} errors={buildFieldErrors.items[index]} disabled={programmingControlsDisabled} onChange={(update) => updateExercise(index, update)} onMove={moveExercise} onRemove={removeExercise} />)}
                  </div>
                </div>
...
function DraftExerciseCard({ item, index, total, errors, disabled, onChange, onMove, onRemove }: { item: DraftExercise; index: number; total: number; errors?: BuildFieldErrors["items"][number]; disabled: boolean; onChange: (update: Partial<DraftExercise>) => void; onMove: (index: number, direction: -1 | 1) => void; onRemove: (index: number) => void }) {
  ...
  return <article className="rounded-2xl border border-slate-200 bg-white p-4">
    ...
    <div className="mt-4 grid gap-4 sm:grid-cols-2">
      <label className="block"><span className="mb-1.5 block text-sm font-semibold text-slate-700">Sets</span><input type="number" inputMode="numeric" min="1" step="1" value={item.targetSets} onChange={(event) => onChange({ targetSets: event.target.value })} disabled={disabled} className="min-h-12 w-full rounded-xl border border-slate-200 bg-stone-50 px-3 text-base font-medium outline-none focus:border-teal-600 focus:bg-white focus:ring-2 focus:ring-teal-600/15 disabled:bg-slate-100" />{errors?.sets && <FieldError>{errors.sets}</FieldError>}</label>
      <label className="block"><span className="mb-1.5 block text-sm font-semibold text-slate-700">RPE <span className="font-normal text-slate-500">optional</span></span><input type="number" inputMode="decimal" min="1" max="10" step="0.5" value={item.targetRpe} onChange={(event) => onChange({ targetRpe: event.target.value })} disabled={disabled} className="min-h-12 w-full rounded-xl border border-slate-200 bg-stone-50 px-3 text-base font-medium outline-none focus:border-teal-600 focus:bg-white focus:ring-2 focus:ring-teal-600/15 disabled:bg-slate-100" />{errors?.rpe && <FieldError>{errors.rpe}</FieldError>}</label>
    </div>
