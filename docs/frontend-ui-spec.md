...
|---|---|---|---|
| `/coach/calendar` | **Primary** | `GET /exercises`, `POST /workouts`, `POST /scheduled-workouts`, `GET /scheduled-workouts` (athleteId omitted → all connected athletes in a date range) | Default landing page after Coach login and the primary programming workspace. Pick a date and one-or-more Athletes, then either choose an existing saved Workout → Copy & edit (primary) or Assign as saved (secondary), or Build Workout inline → add Exercises → define sets → define a uniform prescription and optional individual-set overrides → Build & Assign. Copy & edit and Inline Build each create one normal saved Workout template only when Build & Assign succeeds, then batch-schedule it; neither creates a Workout per Athlete nor mutates the source. Shows each Athlete's completion status per day (`session` field on the scheduled-workout list). Entry point into `/session/[id]` for review or live 1:1 coaching. |
...
| Route | Role | Backs onto | Purpose |
|---|---|---|---|
| `/today` | Primary (Athlete's only real destination in V0.1) | `GET /me/scheduled-workouts?date=`; month grid uses `GET /me/scheduled-workouts?from=&to=` | Athlete's primary training destination. It defaults to the Athlete's **local current date** and allows lightweight previous/next day navigation so the Athlete can view ScheduledWorkouts already assigned for nearby dates. A Today action returns to the local current date. Each selected-date change calls `GET /api/v1/me/scheduled-workouts?date={selectedDate}`. An optional compact month-grid toggle in the header loads `GET /api/v1/me/scheduled-workouts?from=&to=` for the visible grid (Sunday-first, max 42 days) to mark days with scheduled workouts; tapping a day selects that date on the same page. Day ‹ › navigation and Today jump remain. Each exercise is a compact row (name + one-line prescription); Coach cue is collapsed until opened. This is day navigation inside `/today`, not a new Athlete Calendar route. It remains the only primary Athlete route in V0.1. Account deletion is reached from `/settings`, not from a second primary destination. |

### Shared

| Route | Role | Backs onto | Purpose |
|---|---|---|---|
| `/session/[id]` | Coach (connected) + Athlete (self) | `POST /scheduled-workouts/{id}/session`, `GET /sessions/{id}`, `POST/PATCH/DELETE /sessions/{id}/set-logs`, `POST /sessions/{id}/complete` | The Training Session UI. Both roles use the same screen/domain. The default view is a compact **overview** of every active exercise (name, progress, one-line prescription). Logging happens in a **focus** layer for one exercise at a time (compact set table: log, edit, extra, skip-ahead). Focus is local UI state — Back from focus returns to overview, not out of the session. Focus previous/next switches exercises in this session and shows `{current} / {total}`; it is not a second workout. Complete stays reachable from a sticky Finish control on overview. Existing SetLogs are editable only while `ACTIVE`. A `COMPLETED` session is fully read-only (`GET` only). After an Athlete deletes their account, a Coach may still open historical sessions as read-only (`GET /sessions/{id}`); start/complete/set-log are refused. |
...
| `/join/[code]` | Unauthenticated | `GET /invite-codes/{code}/preview`, `POST /invite-codes/{code}/redeem` | Preview → confirm → auth (embedded, no redirect to `/login`) → redeem → `/today`. Email/password create waits for mailbox verification first. Google/Apple do not require the email form; missing provider name opens a name-only confirm card. Product behavior is defined in `docs/mvp-specification.md`, "Coach & Athlete Onboarding — Implemented (V0.1)". |
...
- Existing Workout path: choose a saved Workout, then use primary `Copy & edit` or secondary `Assign as saved`. Copy & edit hydrates the existing Builder with the selected Workout's name, ordered Exercises, set counts, exercise defaults, unit, prescription mode, and sparse per-position overrides. It clearly identifies that the Coach is editing a copy; the source saved Workout and prior ScheduledWorkout snapshots remain unchanged. Build & Assign creates the edited copy as one new saved Workout and schedules it to the selected Athlete(s). Assign as saved retains the existing direct scheduling behavior. **New Workout** (`+ Add Workout`) always follows the Athlete Calendar currently viewed. That Athlete is default-selected and not locked: the Coach may uncheck them and select others. Submit requires at least one Athlete. Both assignment paths submit the same Athlete IDs the Assign-to checkboxes show. `+ Add Workout` never continues a stored draft, including when that draft is for the same Athlete and date. Assigning an existing Workout does not destroy a stored Build draft.
- Inline Build path: build one ordered Workout draft from existing Exercises, validate it, `POST /api/v1/workouts` once, retain the returned `workout.id`, then `POST /api/v1/scheduled-workouts` once with all selected Athlete IDs and the selected date. Each Exercise may include one optional 500-character Coach cue; it is technique/tempo/safety guidance, not the separate custom training target. The Builder keeps one Exercise open at a time, with a concise collapsed summary; newly added Exercises open automatically. **Continue Draft** restores a stored Build draft's original Athlete, date, and checkbox selection. A Continue Draft that was started as New Workout keeps that exact Assign-to set (the Calendar athlete is not re-added if the Coach unchecked them). A Continue Draft that was started as Continue keeps the source Athlete selected and not clearable. When a stored draft exists, `+ Add Workout` asks the Coach to Start new (current Calendar context) or Continue (original draft context). Backdrop and Escape dismiss the dialog without either action. Start new opens an empty builder and does **not** replace the stored draft until the new Build session has a name or at least one exercise. Build & Assign submits the same Athlete IDs the Assign-to checkboxes show.
...
- A copied Workout uses these same uniform-first rules. The Coach may change one copied exercise's set count or default load/reps/RPE without retyping every set; positions with an explicit override continue to follow override semantics. Each exercise may also set a per-exercise weekly `loadIncrement` (`kg`: 0 / 2.5 / 5 / 10; `lb`: 0 / 5 / 10; defaults 2.5 kg / 5 lb), optional weekly `setIncrement` (`0` or `1`, default `0`), and optional weekly `repsIncrement` (`0` or `1`, default `0`). When exactly one Athlete is selected in the Builder, copy/prefill may suggest bumped loads from the last COMPLETED SetLog for that exercise and unit, falling back to the template's planned load when no history exists, bumped set counts from `setIncrement`, and bumped reps from `repsIncrement` when prescription is REPS. Multi-Athlete assign keeps template-written loads, set counts, and reps. **Week view — Repeat this week:** when exactly one Athlete is selected and the visible week has at least one assignment, Coach may repeat the week forward (+7 days). Preview lists each session (source date → target date, workout name, suggested load/set/reps summary, conflict flag when the target date already has assignments). Confirm orchestrates existing `POST /api/v1/workouts` then `POST /api/v1/scheduled-workouts` per session sequentially; source templates and prior snapshots stay unchanged. Partial failure stops with a clear summary and reuses the created-but-unassigned retry pattern when scheduling fails after create. Workout-wide percentage progression and bulk cross-exercise set/load adjustments remain out of scope.
...
- The Session UI associates a normal actual SetLog with a specific frozen planned position while keeping server-assigned `setNumber` as actual chronology. Extra actual sets are allowed and displayed as EXTRA with no target. Planned positions without an actual log remain visibly incomplete; there is no persisted skipped-row UI in V0.1. Existing SetLogs are tappable for edit only in ACTIVE sessions; completed sessions are read-only. The default Session view is a compact overview of all active exercises; logging and per-set edit happen in a one-exercise focus layer that still presents only the frozen effective target for each planned position.
...
- Coach and Athlete share the same Training Session UI/domain wherever the underlying authorization allows it (session start, set-log CRUD). A COMPLETED session is fully read-only.
- An ACTIVE ScheduledWorkout remains incrementally adjustable. Calendar Edit renders existing targets read-only and lets an active-relationship Coach add, soft-remove, or replace Exercises. The Athlete Session view lets the Athlete add a SYSTEM Exercise or one of the ScheduledWorkout Coach's private Exercises with a complete planned-set prescription, and remove/replace only their own additions. Athlete and Coach additions use the same white card as assigned exercises, plus a quiet text label (`自行新增` / `教練臨時新增`); provenance is never communicated by color alone.
- Removed/replaced Exercises leave the active work queue and remain in an expandable adjustment-history region with their original plan, SetLogs, adjustment actor, and replacement relationship. ACTIVE Session detail refreshes every 15 seconds and when the document returns to the foreground; failed background refresh retains the last successful view and local SetLog form input.
- Voice / Video / AI are deferred — not part of any V0.1 route.
- `/today` supports lightweight day navigation and an optional compact month-grid toggle; there is no separate `/athlete/calendar` route.
...
- Day navigation does not introduce a Calendar domain object. Single-day detail uses `GET /me/scheduled-workouts?date=`; the month grid uses the same endpoint's `from`/`to` summary mode.
...
- **Exercise Library slice boundaries** — no Exercise edit/archive, video, description, tags, categories, Warm-Up/Cooldown type, SAQ, Circuit, Questionnaire, Health, progressions, PR behavior, assets, or standalone Exercise-Library Workout Builder. SYSTEM names stay English identity with display-only zh-TW; the catalog seed lives at `apps/api/seeds/system_exercises_v1.sql`. Do not put media into `name`. Baseline: `docs/tasks/2026-09-07-exercise-catalog-baseline.md`. Media columns are a later contract change: `docs/tasks/2026-09-07-exercise-media-attributes.md`.
